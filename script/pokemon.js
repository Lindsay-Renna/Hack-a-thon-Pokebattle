...
function updatePokemonCard(card, data) {
    ...
    descriptionElement.innerHTML = `
        <li>Type: ${data.types.map(typeInfo => typeInfo.type.name).join(', ')}</li>
        <li>Abilities: ${data.abilities.map(abilityInfo => abilityInfo.ability.name).join(', ')}</li>
        <li>HP: ${data.stats.find(stat => stat.stat.name === 'hp').base_stat}</li>
        <li>ATK: ${data.stats.find(stat => stat.stat.name === 'attack').base_stat}</li>
        <li>DEF: ${data.stats.find(stat => stat.stat.name === 'defense').base_stat}</li>
    `;
}
...
function simulateBattle(pokemon1Data, pokemon2Data) {
    ...
    document.querySelector('.battle-result .winner').textContent = `${winnerName.charAt(0).toUpperCase() + winnerName.slice(1)} is the winner!`;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    button.addEventListener('click', async (event) => {
        ...
        const pokemon1Name = document.querySelectorAll('input[name="names"]')[0].value;
        const pokemon2Name = document.querySelectorAll('input[name="names"]')[1].value;
